# Balatro Vocaloid

Adds a Vocaloid themed deck and changes the jokers and icons to be mostly Vocaloid themed, as well as many relevant/inside jokes

## Config

Config can be changed from in game config menu (restart is required for most of them to take effect)

```lua
-- Whether to try change the word Joker to Miku in the games text.
-- Default: true
["change_text"] = true,

-- Whether to change the Joker atlas.
-- Default: true
["change_joker_atlas"] = true,

-- Whether to have some jokers as GIFs.
-- Default: true
["gif_jokers"] = true,

-- Whether to change the Icon atlas.
-- Default: true
["change_icons"] = true,

-- Whether to change the text colours used for the cards.
-- Default: true
["change_text_colours"] = true,
